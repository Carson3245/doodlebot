import { generateWithHuggingFace, generateWithOllama } from './engines.js';
...
export async function createChatReply({
  ...
  if (!trimmed) {
    return applyTone('How can I help?', personality);
  }
  ...
  if (mode === 'huggingface') {
    ...
    try {
      ...
      console.error('Hugging Face generation failed, falling back to rules:', error);
      return generateRuleBasedResponse(normalized, personality);
    }
  }
  ...
  if (mode === 'ollama') {
    ...
    try {
      const raw = await generateWithOllama(prompt, personality.ai?.ollama);
      return cleanOutput(raw, personality);
    } catch (error) {
      console.error('Ollama generation failed, falling back to rules:', error);
      return generateRuleBasedResponse(normalized, personality);
    }
  }
...
function detectKeywordResponse(message, personality) {
  ...
  for (const keyword of personality.keywords ?? []) {
    if (message.includes(keyword.toLowerCase())) {
      return `I can share more about ${keyword}. What would you like to know?`;
    }
  }
  return null;
}

function generateRuleBasedResponse(normalized, personality) {
